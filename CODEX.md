...
cp CODEX.md /path/to/your/project/
```

### Using with Codex CLI

```bash
# Open editor with Codex context
# (Codex will see this file if in working directory)

# For individual completions
cat CODEX.md | codex --context "Consider these principles"
```

### For VS Code Extensions

If using a Codex-based extension (Copilot, GitHub Copilot, etc.):
- Add this file to `.github/copilot-instructions.md` (GitHub Copilot format)
- Or create `.codex/instructions.md` if your tool supports it

---

## When to Relax These Guidelines

- **Trivial completions** (single lines, obvious patterns): Trust Codex
- **Boilerplate** (imports, basic setup): Let Codex generate freely
- **Prototyping** (one-off scripts): Speed over perfection
- **Auto-generated code** (migrations, scaffolding): Different rules apply

For serious work: Follow all four principles.
...
- **Fewer surprises in Codex completions** — Output matches your intent
- **Fewer rejections** — You accept most suggestions without heavy editing
- **Faster integration** — Accepted code works first try
- **Better patterns** — Your file's style is preserved, not overwritten

---

## Examples by Principle

See `EXAMPLES.md` for detailed before/after code examples for each principle.
